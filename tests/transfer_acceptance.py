...
import os, pty, select, subprocess, sys, tempfile, time, tty
    ...
    for receive in (False, True):
        with tempfile.TemporaryDirectory(prefix='tio-transfer-') as root:
            ...
            try:
                ...
                peer = subprocess.Popen(command, stdin=serial,stdout=serial,stderr=subprocess.PIPE,cwd=root if receive else target)
